...
async function updateMainConfig() {
  ...
  try {
    ...
    const updatedContent = content
      .replace(/complexity:\s*\['error',\s*\d+\]/, `complexity: ['error', ${PLUGIN_LIMITS.MAX_COMPLEXITY}]`)
      .replace(/max-lines-per-function:\s*\['error',\s*\{\s*max:\s*\d+/, `max-lines-per-function: ['error', { max: ${PLUGIN_LIMITS.MAX_LINES_PER_FUNCTION}`)
      .replace(/max-params:\s*\['error',\s*\d+\]/, `max-params: ['error', ${PLUGIN_LIMITS.MAX_PARAMS}]`);
...
async function updatePluginCode() {
  ...
  try {
    ...
    const updatedQualityContent = qualityContent
      .replace(/MAX_LINES_PER_FUNCTION:\s*\d+/, `MAX_LINES_PER_FUNCTION: ${PLUGIN_LIMITS.MAX_LINES_PER_FUNCTION}`)
      .replace(/MAX_FUNCTION_COMPLEXITY:\s*\d+/, `MAX_FUNCTION_COMPLEXITY: ${PLUGIN_LIMITS.MAX_COMPLEXITY}`)
