...
test('task creation and execution', async () => {
  ...
  const status = orchestrator.getSystemStatus();
  expect(status.queuedTasks.length).toBeGreaterThan(0);
});
...
test('skill discovery and allocation', async () => {
  ...
  const allocatedSkills = orchestrator.allocateSkillsToTask('test-task', [
    'code-analysis',
    'testing',
  ]);
...
test('resource optimizer skill', async () => {
  ...
  expect(optimization.newAllocations).toBeDefined();
