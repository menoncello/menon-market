...
export class DirectoryStructureGenerator {
     ...
     */
    async generateAgentDirectory(agentType, agentDefinition, outputPath) {
        ...
        try {
            ...
            await this.createDirectoryStructure(outputDir, this.baseDirectoryStructure, templateVars, metrics);
     ...
     */
    generateAgentDirectoryName(agentType, agentId) {
     ...
     */
    async createDirectoryStructure(basePath, structure, templateVars, metrics) {
        for (const [name, item] of Object.entries(structure)) {
            ...
            if (item.type === 'directory') {
                ...
                const content = await this.generateFileContent(item.template, templateVars, item.templateVars);
     ...
     */
    async generateFileContent(templatePath, templateVars, additionalVars) {
        try {
            ...
            content = content.replace(/{{#each (\w+)}}([\s\S]*?){{\/each}}/g, (_match, arrayName, blockContent) => {
                ...
                if (Array.isArray(arrayOrObject)) {
                    return arrayOrObject
                        ...
                        itemContent = itemContent.replace(/{{#unless @last}}([\s\S]*?){{\/unless}}/g, (_conditionalMatch, conditionalContent) => {
            ...
            content = content.replace(/{{#if (\w+)}}([\s\S]*?){{\/if}}/g, (_match, conditionName, blockContent) => {
            ...
            throw new Error(`Failed to generate content from template ${templatePath}: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    /**
     ...
     */
    async validateGeneratedStructure(outputDir, agentType) {
        const errors = [];
        const config = this.agentConfigs[agentType];
        try {
...
{{devDependencies}}
  ...
  "exclude": [
    ...
    generatePrettierConfigTemplate() {
        return `{
  "semi": true,
  "trailingComma": "es5",
  "singleQuote": true,
  ...
  "proseWrap": "preserve"
    ...
    generatePrettierConfigJsTemplate() {
...
//# sourceMappingURL=DirectoryStructureGenerator.js.map