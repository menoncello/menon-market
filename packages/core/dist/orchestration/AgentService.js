...
export class AgentService {
    ...
    async findBestAgent(task, requiredTools) {
        const availableAgents = Array.from(this.registeredAgents.values()).filter(agent => this.isAgentSuitableForTask(agent, task, requiredTools));
     ...
     * @param agentId
     */
    async isAgentAvailable(agentId, runningTasks) {
        const agent = this.registeredAgents.get(agentId);
        if (!agent)
            return false;
        // Check if agent is at capacity
        const runningTasksCount = Array.from(runningTasks.values()).filter(task => task.agentId === agentId).length;
     ...
     * @param requiredTools
     */
    isAgentSuitableForTask(agent, task, requiredTools) {
        // Check tool requirements
        if (requiredTools) {
            const hasAllTools = requiredTools.every(tool => agent.configuration.capabilities.allowedTools.includes(tool));
            if (!hasAllTools)
                return false;
        }
     ...
     * @param requiredTools
     */
    scoreAgentForTask(agent, task, requiredTools) {
        let score = 0;
        // Role alignment score
        score += this.getRoleTaskAlignment(agent.role, task);
        // Tool availability score
        if (requiredTools) {
            const availableRequiredTools = requiredTools.filter(tool => agent.configuration.capabilities.allowedTools.includes(tool));
     ...
     * @param task
     */
    getRoleTaskAlignment(role, task) {
