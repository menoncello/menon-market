...
Will update existing story file rather than creating new one.
  ...
  <step n="7" goal="Create or update story document">
    ...
    <step n="7.5" goal="Validate code examples in story">
      <action>Scan all code examples in the generated story</action>
      <action>For each code example:
        - Check TypeScript types would compile in strict mode
        - Verify ESLint compliance (0 errors)
        - Check proper formatting
        - Ensure no eslint-disable or @ts-ignore comments
      </action>
      <check if="code examples have quality issues">Fix examples before saving story</check>
    </step>
  </step>
